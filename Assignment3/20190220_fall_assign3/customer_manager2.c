...
DestroyCustomerDB(DB_T d)
{
  ...
  if(d != NULL && d->ht_id != NULL && d->ht_name != NULL && d->ht_id->array != NULL && d->ht_name->array != NULL){
		 ...
		 const char *name, const int purchase)
{
  ...
  for(search_id = d->ht_id->array[h_id]; search_id != NULL; search_id = search_id->nextOfId){
    if(strcmp(search_id->id, id) == 0) return -1;
  }
  
  struct UserInfo *search_name;
  for(search_name = d->ht_name->array[h_name]; search_name != NULL; search_name = search_name->nextOfName){
    if(strcmp(search_name->name, name) == 0) return -1;
  }
...
UnregisterCustomerByID(DB_T d, const char *id)
{
  ...
  if(d->ht_id->array[h_id]->id == NULL) return -1;
    ...
    if(strcmp(d->ht_name->array[h_name]->name, name) == 0){
      ...
      for(search_name = d->ht_name->array[h_name]; search_name != NULL; search_name = next_name){
    ...
    for(search_id = d->ht_id->array[h_id]; search_id != NULL; search_id = next_id){
      next_id = search_id->nextOfId;
      if(next_id == NULL) return -1;
...
GetSumCustomerPurchase(DB_T d, FUNCPTR_T fp)
{
  ...
  int sum = 0;
  for(int i=0; i<BUCKET_COUNT; i++){
    for(struct UserInfo *search = d->ht_id->array[i]; search != NULL; search = search->nextOfId){
...
//Compile : gcc209 -D_GNU_SOURCE -o testclient2 testclient.c customer_manager2.c