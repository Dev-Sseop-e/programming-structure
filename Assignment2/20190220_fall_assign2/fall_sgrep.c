...
DoFind(const char *pcSearch)
{
  ...
  if (StrGetLength(pcSearch) > MAX_STR_LEN) {
    fprintf(stderr, "Error: input argument is too long\n");
    return FALSE;
  }
...
DoReplace(const char *pcString1, const char *pcString2)
{
  ...
  int S1 = StrGetLength(pcString1);
  int S2 = StrGetLength(pcString2);

  if (S1 > MAX_STR_LEN || S2 > MAX_STR_LEN) {
    fprintf(stderr, "Error: input argument is too long\n");
    return FALSE;
  }
  ...
  while(fgets(buf, sizeof(buf), stdin)){
    ...
    if (StrSearch(buf, pcString1) != '\0') StrCopy(StrSearch(buf, pcString1), pcString2);
